...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a IVY address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no IVY: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert ivy:// to ivy:
    //
    //    Cannot handle this later, because ivy:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open ivy.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "IVY.lnk";
}
...
{
    return GetAutostartDir() / "ivy.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ivy.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ivy app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ivy app to startup item list
