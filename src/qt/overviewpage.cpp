...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeIvyAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIvyAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeIvyAmount = strAnonymizeIvyAmount.remove(strAnonymizeIvyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIvyAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeIvyAmount * COIN) nMaxToAnonymize = nAnonymizeIvyAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeIvyAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeIvyAmount));
        strAnonymizeIvyAmount = strAnonymizeIvyAmount.remove(strAnonymizeIvyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIvyAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
